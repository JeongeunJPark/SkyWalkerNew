Shader "Minimalist/LWRP/Standard Localspace"
{
	Properties
	{
		//Texture Module
	...
	SubShader
	{
		...
		Pass {
			...
			vertexOutput vert(vertexInput input){
				...
				half dirFront  = max(dot(input.normalOS, FrontDir),  0.0);
				half dirBack   = max(dot(input.normalOS, BackDir),   0.0);
				half dirLeft   = max(dot(input.normalOS, LeftDir),   0.0);
				half dirRight  = max(dot(input.normalOS, RightDir),  0.0);
				half dirTop    = max(dot(input.normalOS, TopDir),    0.0);
				half dirBottom = max(dot(input.normalOS, BottomDir), 0.0);
				...
				float4 GradPos = float4(input.positionOS.xyz, 1);
