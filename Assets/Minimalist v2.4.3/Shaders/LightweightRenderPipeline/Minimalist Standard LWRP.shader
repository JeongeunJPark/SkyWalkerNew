Shader "Minimalist/LWRP/Standard"
{
	Properties
	{
		//Texture Module
	...
	SubShader
	{
		...
		Pass {
			...
			vertexOutput vert(vertexInput input){
				...
				output.positionWSFF = float4(vertexInput.positionWS, 0);
				...
				half dirFront  = max(dot(vertexNormalInput.normalWS, FrontDir),  0.0);
				half dirBack   = max(dot(vertexNormalInput.normalWS, BackDir),   0.0);
				half dirLeft   = max(dot(vertexNormalInput.normalWS, LeftDir),   0.0);
				half dirRight  = max(dot(vertexNormalInput.normalWS, RightDir),  0.0);
				half dirTop    = max(dot(vertexNormalInput.normalWS, TopDir),    0.0);
				half dirBottom = max(dot(vertexNormalInput.normalWS, BottomDir), 0.0);
				...
				float4 GradPos = float4(vertexInput.positionWS, 1);
				...
				//Apply Unity fog
					...
					output.positionWSFF = float4(vertexInput.positionWS, fogFactor);
				#endif
			...
			half4 frag(vertexOutput input) : SV_Target{
				...
				#if UNITY_FOG
					float fogFactor = input.positionWSFF.w;
